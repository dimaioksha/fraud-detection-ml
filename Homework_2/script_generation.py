...
def generate_dataset(n_customers = 10000, n_terminals = 1000000, nb_days=90, start_date="2018-04-01", r=5, r_seed=0):
    
    start_time=time.time()
    customer_profiles_table = generate_customer_profiles_table(n_customers, random_state = r_seed)
    print("Time to generate customer profiles table: {0:.2}s".format(time.time()-start_time))
    
    start_time=time.time()
    terminal_profiles_table = generate_terminal_profiles_table(n_terminals, random_state = r_seed)
...
if __name__ == "__main__":
    ...
    subprocess.call(["hdfs", "dfs", "-put", f"{os.path.abspath(full_path)}", f"{file_name}"])
    ...
    with open("output_log.txt", "a") as file:
        file.write(f"Partiton {i+1} saved at {file_name}. Time spent: {str_time}\n")
